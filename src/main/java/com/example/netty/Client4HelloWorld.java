	...
	public ChannelFuture doRequest(String host, int port) throws InterruptedException{
		...
		this.bootstrap.handler(new ChannelInitializer<SocketChannel>() {
			...
			protected void initChannel(SocketChannel ch) throws Exception {
				 ...
				 * 1、固定数据长度，解码
				 */
				// 信道处理，加一个固定长度的解码器，缓存中每三个字节读取一次，不足三个字节的，暂存在客户端缓存中。
				// 既然是解码器，当然是用于接收数据。至于发送数据时的编码，不知道是自己组织，还是netty也有编码器类。
				channelHandler[0] = new FixedLengthFrameDecoder(30);
				channelHandler[1] = new Client4HelloWorldHandler();
				/*ch.pipeline().addLast("heartbeat",new IdleStateHandler(30, 30, 30) {
	...
	public static void main(String[] args) {
		...
		try{
			...
			while(true){
				...
				//查看是否断线，断线重连
				if(future == null){
					future = future = client.doRequest("localhost", 9999);
					System.out.println("重连1");
				}
				if(!future.channel().isActive()){
					future = future = client.doRequest("localhost", 9999);
					System.out.println("重连2");
				}
				//netty中发送的数据，必须是bytebuf类型，大概发送的是一个缓冲区
				future.channel().writeAndFlush(Unpooled.copiedBuffer(line.getBytes("UTF-8")));
